<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="text-gray-900 font-medium">Modifier l'entretien</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="card p-6">
            ...
            <div class="mb-6">
                <h2 class="text-base font-semibold text-gray-900">Modifier l'entretien</h2>
            ...
            <form method="POST" action="{{ route('interviews.update', $interview) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="type"
                            ...
                            @foreach(App\Models\Interview::TYPE_LABELS as $value => $label)
                                <option value="{{ $value }}"
                                        {{ old('type', $interview->type) === $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="result"
                            ...
                            @foreach(App\Models\Interview::RESULT_LABELS as $value => $label)
                                <option value="{{ $value }}"
                                        {{ old('result', $interview->result) === $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('applications.show', $interview->application_id) }}" class="btn-secondary">
                        Annuler
                    </a>
                </div>
